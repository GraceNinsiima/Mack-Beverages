...
int numbottles300mlfilled=0;
int numbottles500mlfilled=0;
long int thenumber;
time_t start;
...
void litreslcd(){
	...
	lcd_write_int(numbottles300mlfilled);
	lcd_position(3);
	lcd_write_string("500ml filled- ");
	lcd_write_int(numbottles500mlfilled);
}
...
void saveSetting(){
	eeprom_write_float(&my_eeprom_array[0], (float)numbottles300mlfilled);
	eeprom_write_float(&my_eeprom_array[1], (float)numbottles500mlfilled);
	eeprom_write_float(&my_eeprom_array[2], totalnumlitres);
	eeprom_write_float(&my_eeprom_array[3], numApples);
	eeprom_write_float(&my_eeprom_array[4], numMangoes);
	eeprom_write_float(&my_eeprom_array[5], numGuavas);
	eeprom_write_float(&my_eeprom_array[6], numOranges);
...
void resumeSetting(){
	numbottles300mlfilled=(int)eeprom_read_float(&my_eeprom_array[0]);
	numbottles500mlfilled=(int)eeprom_read_float(&my_eeprom_array[1]);
...
if (totalnumlitres>(bottlesize/1000))
{
	...
	if (bottlesize==300)
	{
		...
		numbottles300mlfilled=numbottles300mlfilled+1;
	}
	else{
	numbottles500ml=numbottles500ml-1;
	numbottles500mlfilled=numbottles500mlfilled+1;
	}
	litreslcd();
...
void adminmenu(){
	...
	TX_NEWLINE;
	TX_NEWLINE;
	transmitString("> Select Option (1-7): ");
